...
# Available Styles
# >> Created and tested on : wofi 1.6.0-1
#
...
# wofi_command="wofi -x11 -theme $dir/full_rounded"
...
# Message
msg() {
	wofi -theme "$dir/message.rasi" -e "Available Options  -  yes / y / no / n"
}

# Variable passed to wofi
options="$shutdown\n$reboot\n$lock\n$suspend\n$logout"

chosen="$(echo -e "$options" | $wofi_command -p "Uptime: $uptime" -dmenu -selected-row 2)"
case $chosen in
    $shutdown)
			systemctl poweroff
        ;;
